...
from tests.schemas import AsyncSchema, Schema
...
async def test_errors_when_missing_operation_name(client, url_builder):
    response = await client.get(
        ...
        subscription TestSubscriptions { subscriptionsTest }
        """
        )
    )
...
@pytest.mark.asyncio
async def test_supports_post_json_query_with_get_variable_values(client, url_builder):
    response = await client.post(
...
async def test_passes_request_into_request_context(client, url_builder):
    ...
    assert response.status == 200
    assert await response.json() == {"data": {"request": "testing"}}
...
class TestAsyncSchema:
    ...
    @pytest.mark.asyncio
    async def test_AsyncSchema(self, client, url_builder):
